...
function DiscoverImage({ title, type }: { title: string; type: string }) {
  ...
  if (!url) return (
    <div className="w-full h-full bg-surface-2 flex items-center justify-center text-sm text-muted text-center p-2 uppercase font-bold">
      {type}
    </div>
  );
    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row gap-8 relative">
        ...
        <div className={`${showFilters ? 'block' : 'hidden'} md:block w-full md:w-64 space-y-6 flex-shrink-0 bg-surface md:bg-transparent border md:border-0 border-border p-4 md:p-0 rounded-2xl md:rounded-none`}>
            <div className="hidden md:flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="text-sm font-bold text-muted hover:text-danger transition-colors flex items-center gap-1"
              >
            ...
            <div className="space-y-3">
              <label className="text-sm font-bold uppercase tracking-widest text-muted">Media Type</label>
              <div className="flex flex-wrap gap-2">
                {(['all', 'movie', 'tv', 'game'] as MediaType[]).map((t) => (
                  <button
                    key={t}
                    onClick={() => setMediaType(t)}
                    className={`flex-1 px-3 py-2 rounded-lg text-sm font-bold border transition-all ${mediaType === t ? 'bg-primary/20 border-primary text-primary' : 'bg-surface-2 border-border text-muted hover:border-text/30'}`}
                  >
            ...
            {/* Sort */}
            <div className="space-y-3">
              <label className="text-sm font-bold uppercase tracking-widest text-muted">Sort By</label>
              <select
                value={sortBy}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-sm font-bold uppercase tracking-widest text-muted">Genres</label>
                {selectedGenres.length > 0 && (
                  <button onClick={() => setSelectedGenres([])} className="text-sm text-muted hover:text-danger">Clear</button>
                )}
              </div>
                ...
                {genres.map(g => {
                   ...
                   return (
                     <button
                       ...
                       className={`px-2 py-1 rounded-md text-sm font-bold border transition-all ${isSelected ? 'bg-primary-2/20 border-primary-2 text-primary-2' : 'bg-surface-2 border-border/50 text-muted hover:border-text/30'}`}
                     >
                       {g.name}
                     </button>
                   )
            ...
            <div className="md:hidden flex justify-end pt-4 border-t border-border mt-6">
               <button
                 onClick={clearFilters}
                 className="flex items-center gap-2 text-sm font-bold text-muted hover:text-danger transition-colors mr-4"
               >
                 <X size={14} /> Clear All
               </button>
               <button
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {mediaType !== 'all' && (
            <span className="flex items-center gap-1.5 px-3 py-1 bg-primary/10 border border-primary/30 text-primary rounded-full text-sm font-bold uppercase tracking-tighter">
              Type: {mediaType}
              <X size={10} className="ml-1 cursor-pointer" onClick={() => setMediaType('all')} />
            </span>
          )}
          {selectedGenres.length > 0 && (
            <span className="flex items-center gap-1.5 px-3 py-1 bg-primary-2/10 border border-primary-2/30 text-primary-2 rounded-full text-sm font-bold uppercase tracking-tighter">
        ...
        <div className="relative" ref={searchContainerRef}>
              ...
              {suggestions.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 truncate">
                    ...
                    {item.release_year && <span className="text-sm text-muted ml-2">({item.release_year})</span>}
                  </div>
                  <span className="text-sm uppercase font-bold px-2 py-0.5 rounded-md bg-surface-2 text-muted border border-border/50">
                    {item.type}
                  </span>
          ...
          {!query.trim() && recentSearches.length > 0 && (
             <div className="mt-4 flex flex-wrap items-center gap-2">
                <span className="text-sm font-bold text-muted flex items-center gap-1 mr-2"><Clock size={12}/> Recent:</span>
                {recentSearches.map((term, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleRecentSearchClick(term)}
                    className="px-3 py-1 rounded-full text-sm bg-surface-2 border border-border/50 text-text hover:border-primary/50 hover:text-primary transition-colors"
                  >
                    {term}
                  </button>
                ))}
                <button
                  ...
                  className="ml-auto text-sm text-muted hover:text-danger flex items-center gap-1"
                >
                  Clear History
                </button>
             </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {results.map((item) => (
                ...
                {item.cover_url ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-sm text-muted bg-surface">
                ...
                <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
                  <span className="text-sm uppercase font-bold px-2 py-0.5 rounded-full backdrop-blur-md bg-black/60 text-white border border-white/10">
                    {item.type}
                  </span>
                  {item.rating > 0 && (
                    <span className="text-sm font-bold px-2 py-0.5 rounded-full backdrop-blur-md bg-yellow-500/20 text-yellow-500 border border-yellow-500/30">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4 z-20">
                  ...
                  <p className="text-sm text-muted line-clamp-3 mb-4 italic leading-relaxed">
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-primary hover:bg-primary/90 text-white rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50 relative z-30"
                  >
        ...
        <div className="flex items-center justify-between">
          ...
          {!discoverLoading && (
            <button 
              ...
              className="text-sm font-bold text-primary hover:text-primary-2 transition-colors uppercase tracking-widest"
            >
              Refresh Intel
            </button>
          )}
                ...
                <div className="flex-1 flex flex-col justify-between py-1">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-sm uppercase font-bold px-2 py-0.5 rounded-full bg-primary-2/20 text-primary-2 border border-primary-2/30">
                        {item.type}
                      </span>
                      <span className="text-sm uppercase font-bold px-2 py-0.5 rounded-full bg-surface-2 text-muted border border-border">
                        {item.genre}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-muted leading-relaxed line-clamp-2 italic">
                      "{item.description}"
                    </p>
                  </div>
                  <button
                    ...
                    disabled={addedIds.has(item.title)}
                    className="mt-4 flex items-center justify-center gap-2 py-2 px-4 bg-primary-2/10 hover:bg-primary-2 text-primary-2 hover:text-black rounded-xl text-sm font-bold transition-all border border-primary-2/20 disabled:opacity-50"
                  >
                    {addedIds.has(item.title) ? (
                      <><Check size={14} /> In Library</>
                    ) : (
