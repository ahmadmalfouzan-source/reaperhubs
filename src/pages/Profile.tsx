  ...
  if (!data || !data.user) {
    return (
      <div className="text-center py-32 space-y-6">
        ...
        <button onClick={() => navigate('/dashboard')} className="px-8 py-3 bg-primary text-black font-bold rounded-xl uppercase text-sm md:text-xs tracking-widest hover:scale-105 transition-all">
          Return to HQ
        </button>
      </div>
    );
      ...
      <div className="bg-surface border-2 border-border/50 rounded-[32px] md:rounded-[48px] p-6 md:p-14 flex flex-col md:flex-row gap-8 md:gap-10 items-center md:items-start text-center md:text-left relative overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex-1 relative z-10 pt-2 space-y-6 w-full">
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
              <div className="space-y-1">
                ...
                <p className="text-muted font-mono text-sm md:text-xs md:text-sm">@{user.username}</p>
              </div>
              ...
              {!isCurrentUser && (
                <button
                  ...
                  className={cn(
                    "px-6 md:px-8 py-2.5 md:py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 border-2 shadow-xl text-sm md:text-xs md:text-sm",
                    isFollowing 
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 md:gap-4">
              <div className="flex items-center gap-2 bg-surface-2 px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl border border-border">
                ...
                <div className="flex gap-1 items-baseline">
                  ...
                  <span className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs text-muted uppercase font-bold tracking-widest font-sans">Followers</span>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-surface-2 px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl border border-border">
                ...
                <div className="flex gap-1 items-baseline">
                  ...
                  <span className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs text-muted uppercase font-bold tracking-widest font-sans">Following</span>
                </div>
              </div>
              <span className="px-3 md:px-4 py-1.5 md:py-2 bg-primary/10 text-primary text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold uppercase tracking-[0.2em] rounded-xl md:rounded-2xl border border-primary/20 flex items-center gap-2">
          ...
          <div className="bg-surface-2/30 backdrop-blur-xl border border-border/50 rounded-[24px] md:rounded-[32px] p-6 md:p-8 group/bio relative">
            <h3 className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold uppercase tracking-[0.3em] text-primary-2 mb-3 flex items-center gap-2">
               Classified Dossier
            ...
            {editing && (
              <div className="flex gap-2 mt-4">
                <button 
                  ...
                  disabled={updating}
                  className="px-6 py-2 bg-primary text-black font-bold rounded-xl text-sm md:text-sm md:text-xs uppercase tracking-widest disabled:opacity-50"
                >
                  {updating ? <Loader2 className="animate-spin" /> : "Save Changes"}
                </button>
                <button 
                  onClick={() => setEditing(false)}
                  className="px-4 py-2 text-muted hover:text-white transition-colors text-sm md:text-sm md:text-xs font-bold uppercase tracking-widest"
                >
                  Abort
                </button>
              </div>
            ...
            {isCurrentUser && !editing && (
               <button 
                ...
                className="absolute top-8 right-8 text-sm md:text-xs md:text-[10px] font-bold uppercase text-muted hover:text-primary transition-all opacity-0 group-hover/bio:opacity-100"
               >
                 Recalibrate
               </button>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        ...
        <div className="lg:col-span-1 space-y-8 md:space-y-12">
          <div className="bg-surface-2/30 border border-border/50 rounded-3xl p-6 flex flex-col items-center text-center">
            ...
            <p className="text-sm md:text-xs text-muted mb-4">View your complete performance history, heatmaps, and genre breakdowns.</p>
          ...
          <section className="space-y-6">
              ...
              {achievements.length === 0 ? (
                ...
                achievements.slice(0, 4).map((badge) => (
                  ...
                  >
                    ...
                    <h3 className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold uppercase tracking-widest mb-1 text-white truncate px-1">
                      {badge.title}
                    </h3>
                    {!badge.unlocked && (
            ...
            <button className="w-full py-3 md:py-4 text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold uppercase tracking-[0.3em] text-muted hover:text-primary transition-all border border-dashed border-border rounded-xl md:rounded-2xl">
              View All Milestones
            </button>
          </section>

          <section className="bg-surface border border-border rounded-[24px] md:rounded-[32px] p-6 md:p-8 space-y-6">
              <h3 className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold uppercase tracking-[0.2em] text-muted">Field Logistics</h3>
              <div className="space-y-4">
                  <div className="flex justify-between items-center px-4 py-3 bg-surface-2 rounded-xl md:rounded-2xl">
                      <span className="text-sm md:text-sm md:text-xs font-bold text-muted uppercase">Credits</span>
                  ...
                  <div className="flex justify-between items-center px-4 py-3 bg-surface-2 rounded-xl md:rounded-2xl relative overflow-hidden group">
                      ...
                      <span className="text-sm md:text-sm md:text-xs font-bold text-muted uppercase">XP Protocol</span>
        ...
        <div className="lg:col-span-2 space-y-8 md:space-y-12">
          {isCurrentUser && (
            <section className="space-y-6">
              <div className="flex items-center justify-between border-b border-border/50 pb-4">
                ...
                <button onClick={() => navigate('/library')} className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold text-primary uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all">
              ...
              {library.length === 0 ? (
                <div className="bg-surface/50 border border-dashed border-border p-8 md:p-12 rounded-[24px] md:rounded-[32px] text-center space-y-4">
                  ...
                  <p className="text-muted italic text-sm md:text-xs md:text-sm">No data points saved in active archive.</p>
                  <button onClick={() => navigate('/search')} className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs font-bold text-white bg-surface-2 px-5 md:px-6 py-2 rounded-full uppercase tracking-tighter border border-border hover:border-primary transition-all">
                    Search Registry
                  </button>
                </div>
              ) : (
                  ...
                  {library.slice(0, 3).map((item) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-3 md:bottom-4 left-3 md:left-4 right-3 md:right-4">
                        ...
                        <p className="text-sm md:text-sm md:text-xs md:text-sm md:text-xs font-bold text-white line-clamp-1 italic">{item.media_items?.title}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <section className="space-y-6">
            <div className="flex items-center justify-between border-b border-border/50 pb-4">
              ...
              <span className="text-sm md:text-xs md:text-[10px] md:text-sm md:text-sm md:text-xs text-muted font-bold uppercase tracking-widest">{posts.length} Transmissions</span>
            </div>
            {posts.length === 0 ? (
              <div className="bg-surface/50 border border-border rounded-[24px] md:rounded-[32px] p-8 md:p-10 text-center space-y-4">
                ...
                <p className="text-muted text-sm md:text-xs md:text-sm italic">Zero radio chatter detected from this origin.</p>
              </div>
            ) : (
              <div className="space-y-4 md:space-y-6">
                {posts.map((post: any) => (
                  <div key={post.id} className="bg-surface border-l-4 border-primary border border-border rounded-xl md:rounded-2xl p-6 md:p-8 hover:bg-surface-2/50 transition-all duration-300 shadow-lg">
                    <p className="mb-4 text-text/90 italic leading-relaxed font-medium text-sm md:text-xs md:text-base">"{post.content}"</p>
