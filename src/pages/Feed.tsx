    ...
    <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in duration-700">
      <div className="lg:col-span-2 space-y-8">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20 text-sm font-bold text-primary uppercase tracking-[0.2em] mb-2">
            Collective Pulse
          </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
                  {TAGS.map((tag) => (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2 px-4 py-2 rounded-2xl border text-sm font-bold uppercase tracking-widest transition-all whitespace-nowrap",
                        selectedTag === tag.id 
            ...
            {items.map((item) => (
              <div key={item.id} className="group bg-surface border-2 border-border/30 hover:border-primary/50 shadow-2xl rounded-[48px] p-8 md:p-10 transition-all duration-500 hover:-translate-y-1">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm md:text-sm text-muted mt-1.5 font-bold uppercase tracking-widest">
                    ...
                    {menuOpenId === item.id && user && item.user_id === user.id && (
                      <div className="absolute right-0 mt-2 w-48 bg-surface border border-border shadow-2xl rounded-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                        <button 
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold uppercase tracking-widest text-muted hover:text-primary hover:bg-primary/5 transition-all"
                        >
                          <Edit3 size={14} />
                          Edit Signal
                        </button>
                        <button 
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold uppercase tracking-widest text-muted hover:text-danger hover:bg-danger/5 transition-all border-t border-border/50"
                        >
                          <Trash2 size={14} />
                          Abort Trans
                        </button>
                ...
                <div className="pl-1 space-y-6">
                      ...
                      <form onSubmit={handleUpdatePost} className="space-y-4 animate-in fade-in duration-300">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="bg-primary hover:bg-primary/90 text-black font-bold rounded-xl px-6 py-2 text-sm uppercase tracking-widest transition-all disabled:opacity-50 flex items-center gap-2"
                          >
                          ...
                          <button
                            ...
                            className="bg-surface-2 border border-border text-muted hover:text-white font-bold rounded-xl px-6 py-2 text-sm uppercase tracking-widest transition-all flex items-center gap-2"
                          >
                            <X size={14} />
                            Cancel
                          </button>
                  ...
                  {item.media_type && (
                     <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary-2/10 border border-primary-2/20 rounded-xl text-sm font-bold text-primary-2 uppercase tracking-[0.2em] shadow-lg">
                  ...
                  <div className="flex flex-wrap items-center gap-6 pt-8 border-t border-border/30">
                    ...
                    >
                      ...
                      <span className="text-sm font-bold leading-none">{item.like_count || 0}</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-sm font-bold leading-none">{item.comment_count || 0}</span>
                    </button>
                    ...
                    <button className="flex items-center gap-3 px-5 py-3 text-muted hover:text-success hover:bg-success/5 rounded-2xl transition-all group/stat border border-transparent hover:border-success/10 ml-auto">
                      ...
                      <span className="text-sm font-bold uppercase tracking-widest hidden sm:inline">Broadcast</span>
                    </button>
                  </div>
      ...
      <div className="hidden lg:block space-y-10 sticky top-28 h-fit">
            ...
            {['#REAPERHAB', '#SUMMER_SLAY', '#TV_INTEL', '#GAME_ARCHIVE'].map((tag, i) => (
              <div key={tag} className="group/tag cursor-pointer flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-muted/30 font-mono text-sm font-bold">0{i+1}</span>
                ...
                <div className="flex flex-col items-end">
                   <span className="text-sm text-muted font-mono uppercase">Signal</span>
                   <span className="text-sm text-primary-2 font-display font-bold">+ {Math.floor(Math.random() * 200)}%</span>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full py-4 text-sm font-bold uppercase tracking-[0.3em] text-muted hover:text-primary transition-all bg-surface-2/50 rounded-2xl border border-border/50 hover:border-primary/30">
            Monitor All Trends
          </button>
        </section>
        ...
        <section className="bg-surface border-2 border-border/50 rounded-[40px] p-8 space-y-8 shadow-2xl">
          ...
          <button className="w-full py-4 text-sm font-bold uppercase tracking-[0.3em] text-success border border-success/20 rounded-2xl hover:bg-success/5 transition-all shadow-lg active:scale-95 transform">
            Secure Recruitment
          </button>
        </section>
      </div>
